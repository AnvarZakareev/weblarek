...
`protected buttonPay: HTMLButtonElement` - хранит кнопку "Оплатить"
...
##### Класс Success
Класс сообщающий об успешной оплате

Конструктор:
`protected events: IEvents, container: HTMLElement` - принемает действие вызываемое при клике на кнопку "За новыми покупками!", и HTML элемент который передает в класс Component

Поля класса:
`protected buttonRestart: HTMLButtonElement` - хранит кнопку "За новыми покупками!"
