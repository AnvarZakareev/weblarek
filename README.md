...
Методы класса:
`getProducts(): Promise<IProduct[]>` — c помощью метода get класса Api и будет получать с сервера объект с массивом товаров
`sendOrder(order: IOrder): Promise<void>` — метод post класса Api, чтобы класс мог отправлять на сервер данные о покупателе и выбранных товарах
