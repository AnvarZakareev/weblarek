...
<!-- - описать конструктор класса и принимаемые параметры (если они есть). Параметры конструктора нужно указать с типами принимаемых данных; -->
Конструктор:
`constructor(ProductArray: IProduct[])` - В конструктор передается массив ВЫБРАННЫХ всех товаров

<!-- - описать поля класса. Для каждого нужно указать тип данных, который будет сохранён в поле класса. Затем описать поле класса — что оно хранит и за что отвечает; -->
Поля класса:
`ProductArrayInBasket: IProduct[]` - хранит массив товаров, выбранных покупателем для покупки.
...
<!-- - описать конструктор класса и принимаемые параметры (если они есть). Параметры конструктора нужно указать с типами принимаемых данных; -->
Конструктор:
`constructor(Buyer: IBuyer[])` - В конструктор передается массив данных пользователя для оформления покупки

<!-- - описать поля класса. Для каждого нужно указать тип данных, который будет сохранён в поле класса. Затем описать поле класса — что оно хранит и за что отвечает; -->
Поля класса:
`Buyer: IBuyer[]` - хранит массив данных пользователя для оформления покупки.
...
`getBuyerData(IBuyer): IBuyer[]` - получение всех данных покупателя;
`clearBuyerData(IBuyer): IBuyer[]` - очистка данных покупателя;
`validBuyerData(IBuyer[]): boolean` - валидация данных.
  ...
  Поле является валидным, если оно не пустое. 
    Подсказка: Как один из вариантов решения задачи валидации может быть метод, который вернет объект. В объекте могут присутствовать поля, соответствующие полям класса, значениями у которых будет текст ошибки. Если же поле не содержит ошибок, то такое свойство в объекте может отсутствовать.
      ...
      Такой объект будет сообщать о валидности полей с телефоном и адресом и наличии ошибок в полях с видом оплаты и емэйлом. Можно сделать и отдельные методы для каждого поля, выбор за вами.
  Метод валидации должен давать возможность определить не только валидность каждого отдельного поля, 
